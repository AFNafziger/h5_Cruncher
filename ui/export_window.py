...
class ExportWindow:
    ...
    def _setup_ui(self) -> None:
        ...
        search_entry = ttkb.Entry(left_frame, textvariable=self.column_search_var, bootstyle="info")
        search_entry.grid(row=0, column=0, sticky=EW, pady=(0, 10))
        ...
        self.column_list_frame.grid(row=1, column=0, sticky=NSEW)
        self.column_list_frame.grid_columnconfigure(0, weight=1)

        self.column_checkboxes = {} # To store Checkbutton widgets
        self.column_vars = {}      # To store BooleanVar for each checkbox

        select_all_button = ttkb.Button(left_frame, text="Select All (filtered)", command=self._select_all_filtered_columns, bootstyle="info-outline")
        select_all_button.grid(row=2, column=0, sticky=EW, pady=(10, 0))
    ...
    def _populate_column_list(self, columns_to_display: List[str]) -> None:
        ...
        for col in columns_to_display:
            var = ttkb.BooleanVar(value=False) # Default to unselected
        ...
        self._update_selected_columns() # Initial update
